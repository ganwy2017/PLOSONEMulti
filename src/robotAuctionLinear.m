%[auctionTotalU, XNew]=robotAuctionLinear(Nt, Nr, posObj, L, Utility, CR, vel, Kij, typeObj, THAuction, auctionType, Coef, VigParam)
...
%XNew: robot-task assignement following the optimization format result (see robotLinearExperiment function)

function [auctionTotalU, XNew]=robotAuctionLinear(Nt, Nr, posObj, L, Utility, CR, vel, Kij, typeObj, THAuction, auctionType, Coef, VigParam)

        
    function [predictedU predictedT totalCapacityNew] = predictedUtility(totalCapacity, n, robotIndex, taskIndex)
...
%         Coef(7,:) = [399.1302, 1.3657, -2.0857];
        ...
        distance = sqrt((posObj(taskIndex,1).^2)+(posObj(taskIndex,2).^2));
        ...
        totalCapacity;
        totalCapacityNew = totalCapacity + Kij(robotIndex, taskIndex); %(0.5 * (CR(robotIndex,typeObj(taskIndex))*vel(robotIndex)/((vel(robotIndex)*CR(robotIndex,typeObj(taskIndex))) + distance)));
        
        predictedT = L(taskIndex)/(totalCapacityNew-interference);
        predictedU = 1000*Utility(taskIndex)/predictedT;  
    end

 function [totalUtility] = getTotalUtility(taskPerRobot, leadersPerTask)
        XNew = zeros(Nr,Nt);
        totalUtility = 0;
        totalAuxRobots = zeros(1,Nt);
        for j=1:Nt
            %Get the leader
            leaderIndex = leadersPerTask(j);
            nRobots = 1;
            [predictedU predictedT totalCapacity] =  predictedUtility(0,nRobots,leaderIndex,j);
            XNew(leaderIndex,j) = XNew(leaderIndex,j) + 1;
            for i=1:Nr
                if (taskPerRobot(i) ==j)
                    nRobots = nRobots + 1;
                    [predictedU predictedT totalCapacity] =  predictedUtility(totalCapacity,nRobots,i,j);
                    XNew(i,j) = XNew(i,j) + 1;
                end
            end
            totalAuxRobots(j) = totalAuxRobots(j) + nRobots;
            totalUtility = totalUtility + predictedU;
        end
        totalUtility = totalUtility / 1000;
        XNew=reshape(XNew',[1,Nt*Nr]);
    end
   ...
   for j=1:Nt
       ...
       totalCapacity = 0;
       [newUtility newTime totalCapacity] = predictedUtility(totalCapacity,1,leaderIndex,j);
       oldUtility = newUtility;
       n=1;
       while (n<=length(index))
          [newUtility newTime totalCapacity] = predictedUtility(totalCapacity,n+1,index(n),j);
          if ((newTime<=(THAuction)) || (newUtility<=oldUtility))
